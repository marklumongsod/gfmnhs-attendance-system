...
?>
    ...
    <div class="container">
        ...
        <?php
           $trainSchedId= $_GET['trainSchedId'];
            $result = my_query("SELECT * FROM tbl_certificates cert INNER JOIN tbl_trainee_schedules ts ON ts.id=cert.trainee_sched_id INNER JOIN  tbl_trainees t  ON t.id=ts.trainee_id INNER JOIN tbl_schedules s ON s.id=ts.schedule_id WHERE ts.id='$trainSchedId' ");
        for ($i = 1; $row = $result->fetch(); $i++) {
            ...
            <?php
            db_insert('tbl_certificates',
                [
                    "trainee_sched_id" => $trainSchedId,
                    "title" => $_GET['trainSchedId'],
                    "description" => $_GET['trainSchedId'],
                    "cert_date" => $dtNow,
                    "other" => $_GET['trainSchedId'],
